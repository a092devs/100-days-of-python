...
def get_address():
    return [
        address.getText()
        for address in search_results.find_all("a", tabindex="0")
        if address.getText()
    ]
...
def get_price():
    return [
        ...
        for price in soup.find_all(
            "div", class_="StyledPropertyCardDataArea-c11n-8-85-1__sc-yipmu-0 bqsBln"
        )
    ]
...
for i in range(len(get_address())):
    ...
    next_response = driver.find_element(

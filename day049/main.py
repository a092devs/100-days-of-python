...
driver.get(
    "https://www.linkedin.com/jobs/search/?f_AL=true&geoId=102713980&keywords=python%20developer&location=India&refresh=true"
)
time.sleep(5)
...
while jobs_available:
    ...
    try:
        driver.find_element(
            by=By.LINK_TEXT, value=f"{jobs_available[posting_num]}"
        ).click()
        time.sleep(2)
        ...
        try:
            time.sleep(3)
            driver.find_element(
                by=By.CSS_SELECTOR, value='[aria-label="Continue to next step"]'
            ).click()
            time.sleep(3)
            driver.find_element(
                by=By.CSS_SELECTOR, value='[aria-label="Continue to next step"]'
            ...
            time.sleep(15)
            driver.find_element(
                by=By.CSS_SELECTOR, value='[aria-label="Review your application"]'
            ).click()
            time.sleep(3)
            driver.find_element(
                by=By.CSS_SELECTOR, value='[aria-label="Submit application"]'
            ).click()
        except NoSuchElementException:
